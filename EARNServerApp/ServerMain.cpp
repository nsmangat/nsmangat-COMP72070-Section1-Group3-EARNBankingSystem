...
int main(void) {
	...
	while (killSwitch)
	{
		//wait inside loop for connections so there can be a second client after they disconnect
		...
		std::wcout << "Client Connected at socket: " << ConnectionSocket << endl;
		while (connectedToClient) {
			...
			case CREATEACCOUNT:		//When account needs to be created
			{
				...
				AccountInfo newChequingInfo;
				newChequingInfo.clientID = newClient.getObjectID();
				newChequingInfo.accountType = CHEQUINGS;
				newChequingInfo.accountBalance = 0;

				DBAccount newChequingAccount(newChequingInfo);
				responseVal = serverDBWriter.addObject(newChequingAccount);
				AccountInformation newChequing(newChequingAccount.getObjectID(), newChequingAccount.getAccountClientID(), newChequingAccount.getAccountType(), 0);


				AccountInfo newSavingsInfo;
				newSavingsInfo.clientID = newClient.getObjectID();
				newSavingsInfo.accountType = SAVINGS;
				newSavingsInfo.accountBalance = 0;

				DBAccount newSavingsAccount(newSavingsInfo);

				responseVal = serverDBWriter.addObject(newSavingsAccount);

				AccountInformation newSavings(newSavingsAccount.getObjectID(), newSavingsAccount.getAccountClientID(), newSavingsAccount.getAccountType(), 0);

				Packet startup(&newChequing, sizeof(newChequing), &newSavings, sizeof(newSavings), 6);
			...
			case CREDENTIALS:		//Validate username and password
			{
				...
				if (DBSuccess == clientResponse) {
					...
					AccountInformation chequingLogin(clientAccounts[0].getObjectID(), currentClient.getObjectID(), clientAccounts[0].getAccountType(), clientAccounts[0].getAccountBalance());
					AccountInformation savingsLogin(clientAccounts[1].getObjectID(), currentClient.getObjectID(), clientAccounts[1].getAccountType(), clientAccounts[1].getAccountBalance());
			...
			case SHUTDOWN:		//shutdown the server
			{
				...
				//disconnect from client and repeat loop for next
				std::cout << "\n\nDisconnecting from client" << endl;
